...
%>
	...
	<div class="col-1">
		...
		<h4>About ${groups} groups</h4>
			<c:forEach items="${nclones}" begin="1" var="nclone" varStatus="status">	
				<div class="hits-title">
					Class:${status.index} (${nclone} fragments)
				</div>
				<div class="hits-summary">
				<c:set var="sh2" value="${shs[status.index].get(0)}"/>
				<c:set var="status_2" value="1"/>
					<c:set target = "${clone}" property="content" value="${sh2.getContent()}" />
						<div class="hit">			
							...
							<dl class="hit-data">
								...
								<dd>${sh2.getEL()}</dd>
								<dd>  |  </dd>
								<a href="#">Show ${nclone} matches</a>
							</dl>
							<div class="hit-summary">
							<%String[] codes = clone.get_content();%>
								<c:set var = "index" value = "code-content${status.index}+${status_2}" />
